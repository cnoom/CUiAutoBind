...
namespace CUiAutoBind
{
    ...
    {
        private static List<ComponentTypeInfo> cachedTypes;
    ...
    public class SuffixConfigDrawer : PropertyDrawer
    {
        ...
        private static readonly Dictionary<string, DrawerState> stateMap = new Dictionary<string, DrawerState>();
        ...
        private void DrawSearchResultsDropdown(Rect rect, SerializedProperty fullTypeNameProp, SerializedProperty suffixProp, SerializedProperty property)
        {
            ...
            for(var i = 0; i < displayCount; i++)
            {
                ...
                // Tooltip中显示完整信息
                var tooltip = $"{namespaceName}.{typeName}";
        ...
        private void DrawDetailedList(Rect rect, List<ComponentTypeInfo> types, int maxRows)
        {
            ...
            for(var i = 0; i < Mathf.Min(types.Count, maxRows); i++)
            {
                ComponentTypeInfo type = types[i];
                var typeLabel = $"{type.displayName}";
        ...
        private void DrawComponentTypeDropdown(Rect rect, SerializedProperty componentTypeProp, SerializedProperty suffixProp)
        {
            ...
            // 绘制下拉框
            GUIContent[] options = GetComponentTypeOptions();
            string[] stringOptions = Array.ConvertAll(options, x => x.text);
        ...
        {
            Dictionary<string, string> suffixMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                {
                    "Button", "_btn"
                },
        ...
        private GUIContent[] GetComponentTypeOptions()
        {
            ...
            foreach (ComponentTypeInfo componentType in componentTypes)
            {
                // 添加命名空间分隔符
                ...
                var displayName = $"  {componentType.componentType}";
        ...
        private class DrawerState
        {
            ...
            public readonly List<ComponentTypeInfo> recentlyUsed = new List<ComponentTypeInfo>();
            public string searchText = "";
            public bool showAdvancedDropdown;
